...
OUT_DIR = subset_results_path("modelling/lasso")
...
def load_model_data(data_path):
    ...
    if not feature_columns:
        raise ValueError("No usable lasso feature columns were found.")
...
def fold_coefficient_frame(model, feature_columns, fold_id, space_fold, time_fold, n_train, n_test):
    coefficients = pd.DataFrame(
        {
            ...
            COEF_COLUMN: model.named_steps["lasso"].coef_,
        }
    )
    coefficients["abs_coefficient"] = coefficients[COEF_COLUMN].abs()
    coefficients["feature_group"] = coefficients["feature"].map(feature_group)
...
def run_space_time_benchmark(frame, feature_columns):
    ...
    for space_fold, time_fold, train_idx, test_idx in space_time_splits(frame):
        ...
        lasso_metrics = score_predictions(y_test, prediction)
    ...
    return predictions, coefficients, metrics


def summarize_coefficients(coefficients):
    summary = (
        coefficients.groupby(["feature", "feature_group"])
        .agg(
            mean_coefficient=(COEF_COLUMN, "mean"),
            median_coefficient=(COEF_COLUMN, "median"),
            std_coefficient=(COEF_COLUMN, "std"),
            mean_abs_coefficient=("abs_coefficient", "mean"),
            max_abs_coefficient=("abs_coefficient", "max"),
            n_folds=(FOLD_ID_COLUMN, "nunique"),
            nonzero_folds=(COEF_COLUMN, lambda values: int((values.abs() > 0).sum())),
        )
        .reset_index()
    )
    summary["nonzero_share"] = summary["nonzero_folds"] / summary["n_folds"]

    return summary.sort_values("mean_abs_coefficient", ascending=False)


def overall_metrics(predictions):
    lasso_metrics = score_predictions(predictions["observed"], predictions["predicted"])
    baseline_metrics = score_predictions(
...
def plot_space_time_predictions(predictions, fold_metrics, output_dir, prefix="lasso_space_time"):
    ...
    fig.suptitle("Lasso space-time blocked validation")
...
def plot_space_time_coefficients(
    coefficient_summary,
    output_dir,
    max_features=30,
    prefix="lasso_space_time",
):
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    plot_frame = coefficient_summary.head(max_features).copy()
    if plot_frame.empty:
        print("No lasso coefficients to plot.")
        return
    ...
    ax.barh(
        plot_frame["feature"],
        plot_frame["mean_coefficient"],
        xerr=plot_frame["std_coefficient"].fillna(0),
        color=bar_colors,
        alpha=0.9,
    )
    ax.axvline(0, color="black", linewidth=1)
    ax.set_title("Mean lasso coefficients across space-time folds")
    ax.set_xlabel("Mean coefficient after standardizing X")
    ax.set_ylabel("")
    ...
    output_path = output_dir / f"{prefix}_coefficients.png"
    fig.savefig(output_path, dpi=200)
    plt.close(fig)
...
def save_outputs(frame, predictions, coefficients, coefficient_summary, metrics, overall):
    ...
    fold_assignments.to_csv(OUT_DIR / "lasso_space_time_fold_assignments.csv", index=False)
    predictions.to_csv(OUT_DIR / "lasso_space_time_predictions.csv", index=False)
    coefficients.to_csv(OUT_DIR / "lasso_space_time_coefficients_by_fold.csv", index=False)
    coefficient_summary.to_csv(OUT_DIR / "lasso_space_time_coefficient_summary.csv", index=False)
    metrics.to_csv(OUT_DIR / "lasso_space_time_metrics_by_fold.csv", index=False)
    overall.to_csv(OUT_DIR / "lasso_space_time_overall_metrics.csv", index=False)
...
def main():
    ...
    predictions, coefficients, metrics = run_space_time_benchmark(frame, feature_columns)
    coefficient_summary = summarize_coefficients(coefficients)
    overall = overall_metrics(predictions)

    save_outputs(frame, predictions, coefficients, coefficient_summary, metrics, overall)
    plot_space_time_predictions(predictions, metrics, OUT_DIR)
    plot_space_time_coefficients(coefficient_summary, OUT_DIR)
    ...
    print(coefficient_summary.head(20).to_string(index=False))
